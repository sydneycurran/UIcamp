...
const updatePage = async () => {
  ...
  const frArr = fruitsArray.filter((fruit) => {
    return fruit.nutritions.calories < 60;
  })

  const lowCalArr = frArr.map((item) => {
    return item.name;
  })

  const nameARR = fruitsArray.map((item) => {
    return item.name;
  }) 

  const l = lowCalArr.length;
  const x = Math.random();
  const y = Math.floor(x * l);

  // TODO: Create a new HTML element to display your data
  const newElement = document.createElement('div');
  newElement.innerHTML = lowCalArr[y];

  // TODO: Append your new element to the page
  const existingElement = document.getElementById('ui-camp-gallery');
